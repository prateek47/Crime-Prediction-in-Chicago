...
sample.crime = function(crime.csv.path, sample.size, start.month = 1, end.month = 12)
{
  ...
  # keeping only the thefts cases.
  crimes <- crimes[crimes$IUCR == "0810"|
                    crimes$IUCR == "0820"|
                    crimes$IUCR == "0860"|
                    crimes$IUCR == "0865"|
                    crimes$IUCR == "0870"|
  ...
  crimes.dates = strptime(crimes[,"Date"],"%m/%d/%Y %I:%M:%S %p")
...
#' @param end.month Ending month of sample (1-12).
#' @return Data frame containing crime sample with columns for \code{x}-location, \code{y}-location, \code{timestamp}, crime \code{hour}, crime \code{day.of.week}, and crime \code{month}.
sample.crime2 = function(crime.csv.path, sample.size, start.month = 1, end.month = 12)
{
  # read crime data and take sample
  crimes1 = read.csv(crime.csv.path, header=TRUE)
  # keeping only the assault cases
  crimes1 <- crimes1[crimes1$IUCR == "0260"|
                     crimes1$IUCR == "0261"|
                     crimes1$IUCR == "0262(I)"|
                     crimes1$IUCR == "0280"|
                     crimes1$IUCR == "0281(I)"|
  ...
  # reproject lon/lat points to meters
  crimes.locations.lonlat1 = cbind(crimes1$Longitude, crimes1$Latitude)
  crimes.locations.meters1 = project(crimes.locations.lonlat1, proj="+init=epsg:26971")
  
  # convert strings to dates
  crimes.dates1 = strptime(crimes1[,"Date"],"%m/%d/%Y %I:%M:%S %p")
  
  # reassemble more friendly crimes.sample matrix
  crimes1 = as.data.frame(cbind(crimes.locations.meters1[,1],  # x value
                               crimes.locations.meters1[,2],  # y value
                               crimes.dates1,                 # crime date/time
                               hour(crimes.dates1),      # crime hour
                               wday(crimes.dates1),      # crime day of week
                               month(crimes.dates1)))    # crime month
  
  # set column names for convenience
  names(crimes1) = c("x","y","timestamp","hour","day.of.week","month")
  
  # remove NA values
  crimes1 = crimes1[!(is.na(crimes1$x) | is.na(crimes1$y) | is.na(crimes1$timestamp)),]
  
  # filter by month
  crimes1 = crimes1[crimes1$month >= start.month & crimes1$month <= end.month,]
  
  # filter by size
  if(sample.size == -1 | sample.size > nrow(crimes1))
    sample.size = nrow(crimes1)
  
  crimes.sample.rows1 = sample(nrow(crimes1), size=sample.size)
  crimes.sample1 = crimes1[crimes.sample.rows1,]
  
  return (crimes.sample1)
}
...
{
  points.in.boundary = points.within.boundary(points, pointsProj4String, boundary)
  points = points[points.in.boundary,]
  return (points)
}

#' Gets logical indicators of whether points are within a boundary.
#' @param points Matrix (n-by-2) of points in the projection given by \code{pointsProj4String}.
#' @param pointsProj4String Projection string for points.
#' @param boundary Shapefile boundary to use.
points.within.boundary = function(points, pointsProj4String, boundary)
{
...
#' @param max.sample.size Maximum size of the data sample (or -1 for all data).
