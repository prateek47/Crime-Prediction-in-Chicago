...
sample.crime = function(crime.csv.path, sample.size, start.month = 1, end.month = 12)
{
  ...
  # convert factor columns to numbers/integers
  crimes$x = as.numeric(levels(crimes$x)[crimes$x])
  crimes$y = as.numeric(levels(crimes$y)[crimes$y])
  crimes$hour = as.integer(levels(crimes$hour)[crimes$hour])
  crimes$day.of.week = as.integer(levels(crimes$day.of.week)[crimes$day.of.week])
  crimes$month = as.integer(levels(crimes$month)[crimes$month])
...
plot.surveillance.curve = function(threat.prediction, eval.crime.points, prediction.resolution.meters, boundary, add=FALSE)
{
  ...
  idx = order(x)
  auc = round(sum(diff(x[idx])*rollmean(y[idx],2)),digits=3)
...
#' @param max.sample.size Maximum size of the sample points to use (or -1 for all points).
#' @return Vector of estimates of the KDE at \code{est.points}.
